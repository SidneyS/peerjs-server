      ...
      self._checkKey(key, ip, function(err) {
        if (!err) {
          if (!self._clients[key][id]) {
            self._clients[key][id] = { token: token, ip: ip, room: room, id: id };
            self._ips[ip]++;
...
app._notifyAddUser = function(key, target) {
  ...
  __.map(
    __.filter(
      ...
      function (client) {
        return target["room"] === client["room"]
      }
    ),
    function(client) {
...
app._initializeHTTP = function() {
  ...
  this.post('/:key/:id/:token/:room/id', function(req, res, next) {
  ...
  this.get('/:key/peers/:room', function(req, res, next) {
    var key = req.params.key;
    var room  = req.params.room;
    if (self._clients[key]) {
      self._checkAllowsDiscovery(key, function(isAllowed) {
        if (isAllowed) {
          res.send(
            __.map(
              __.filter(self._clients[key], function (c) {
                return c["room"] === room;
              }),
              function(c) {
                return c.id;
  ...
  this.post('/:key/:id/:token/:room/offer', jsonParser, handle);

  this.post('/:key/:id/:token/:room/candidate', jsonParser, handle);

  this.post('/:key/:id/:token/:room/answer', jsonParser, handle);

  this.post('/:key/:id/:token/:room/leave', jsonParser, handle);
};
