      ...
      self._checkKey(key, ip, function(err) {
        if (!err) {
          if (!self._clients[key][id]) {
            Object.keys(self._clients[key]).forEach(
              function(other_id) {
                var other_client = self._clients[key][other_id];
                if (other_client.user_id === user_id) {
                  if (other_client.socket) {
                    other_client.socket.send(
                      JSON.stringify({
                        type: 'FORCE-LOGOUT',
                        payload: {
                          msg: 'Because this user logined-in by other client. user_id="' + user_id + '"'
                        }
                      })
                    );
                    other_client.socket.close();
                    other_client.socket = null;
                  }
                  self._removePeer(key, other_id);
                }
              }
            );
...
app._notifyAddUser = function(key, target) {
  var self = this;
  __.map(
    __.reject(self._clients[key], function(client) {
      return target.id === client.id;
    }),
    function(client) {
      client.socket.send(
        JSON.stringify({
...
app._notifyRemoveUser = function(key, target) {
  var self = this;
  __.map(
    __.reject(self._clients[key], function(client) {
      return target.id === client.id;
...
app._configureWS = function(socket, key, id, token) {
  ...
  var polling = function() {
    if (!(client.ping_id)) {
      client.ping_id = __.now() % Math.pow(10, 6);
      socket.send(
        JSON.stringify({
          type: "PING",
          payload: {
      ...
      client.polling_timer = setTimeout(polling, interval);
    } else {
      client.socket.close();
    }
  }
  if (!!(client.polling_timer)) {
    clearTimeout(client.polling_timer);
  }
  client.polling_timer = setTimeout(polling, interval);
  ...
  socket.on('message', function(data) {
    try {
      ...
      if (['LEAVE', 'CANDIDATE', 'OFFER', 'ANSWER'].indexOf(message.type) !== -1) {
        ...
        if (client.ping_id === message.payload.t) {
          client.ping_id = null;
        }
      } else {
...
app._initializeHTTP = function() {
  ...
  this.post('/:key/:id/:token/:user_id/id', function(req, res, next) {
  ...
  this.get('/:key/peers', function(req, res, next) {
    ...
    if (self._clients[key]) {
      self._checkAllowsDiscovery(key, function(isAllowed) {
        if (isAllowed) {
          res.send(
            Object.keys(self._clients[key]).map(function(id) {
              return {
                id: id,
                user_id: self._clients[key][id]["user_id"]
              };
            })
          );
        } else {
          console.log("Unauthorized error because unknown user accesse to /" + key + "/peers");
          res.sendStatus(401);
        }
      });
    } else {
  ...
  this.get('/:key/peers/:id', function(req, res, next) {
    var key = req.params.key;
    var id  = req.params.id;
    if (self._clients[key]) {
      self._checkAllowsDiscovery(key, function(isAllowed) {
        if (isAllowed) {
          if (!!(client = self._clients[key][id])) {
            res.send({
              id: id,
              user_id: client["user_id"]
            });
          } else {
            client = __.find(self._clients[key], function (c) {
              return c["user_id"] === id;
            });
            if (!!client) {
              res.send({
  ...
  var handle = function(req, res, next) {
    ...
    if (!self._clients[key] || !(client = self._clients[key][id])) {
      if (req.params.retry) {
        self._log("Retry=true", key, id, req.params.token);
        res.sendStatus(401);
        return;
      } else {
        // Retry this request
    ...
    if (req.params.token !== client.token) {
      self._log("Invalid token", key, id, req.params.token);
      res.sendStatus(401);
      return;
    } else {
      self._handleTransmission(key, {
  ...
  this.post('/:key/:id/:token/:user_id/offer', jsonParser, handle);

  this.post('/:key/:id/:token/:user_id/candidate', jsonParser, handle);

  this.post('/:key/:id/:token/:user_id/answer', jsonParser, handle);

  this.post('/:key/:id/:token/:user_id/leave', jsonParser, handle);
};
...
app._removePeer = function(key, id) {
  if (this._clients[key] && (client = this._clients[key][id])) {
    this._ips[this._clients[key][id].ip]--;
    delete this._clients[key][id];
    this._notifyRemoveUser(key, client);
...
app._log = function() {
  if (this._options.debug) {
    console.log.apply(console, __.flatten([ "[" + new Date().toString() + "] ", arguments]));
  }
};
